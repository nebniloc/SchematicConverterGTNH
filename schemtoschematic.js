...
function findUnknownBlocks(arrayBuffer, callback, customMappings) {
  nbt.parse(arrayBuffer, function (error, root) {
    ...
    if (
      ...
      "Palette" in root.value.Schematic.value.Blocks.value
    ) {
    ...
    if ("Palette" in root.value) {
      for (var key in root.value.Palette.value) {
        if (
          !(key in blocksNamespace) &&
          _testConvertToLegacyBlockId(key, customMappings) === 0 &&
          key !== "minecraft:air"
        ) {
          var baseName =
...
function _testConvertToLegacyBlockId(namespaceKey, customMappings) {
  ...
  if (
    ...
    namespaceKey =
      "minecraft:sign" + namespaceKey.substr(namespaceKey.indexOf("[", index));
  }
  ...
  if (~(index = originalKey.indexOf("powered=true"))) {
    ...
    return _testConvertToLegacyBlockId(tempkey);
  }
  ...
  if (customMappings) {
    if (originalKey in customMappings)
      return (
        (customMappings[originalKey].id << 4) | customMappings[originalKey].meta
      );
    var baseName =
      originalKey.indexOf("[") !== -1
        ? originalKey.substr(0, originalKey.indexOf("["))
        : originalKey;
    if (baseName in customMappings)
      return (customMappings[baseName].id << 4) | customMappings[baseName].meta;
  }
  ...
  function convertBlockData(root) {
    ...
    if ("Palette" in root.value && "BlockData" in root.value) {
      ...
      for (var i = 0; i < blockdata.length; i++) {
        ...
        blocks.push(fullBlockId & 0xff);
        data.push(metadata);
        addBlocks.push((fullBlockId >> 8) & 0xf);

        if (fullBlockId > 255) {
          addBlocksNeeded = true;
        }
...
if (typeof module !== "undefined") {
  module.exports = {
    schemtoschematic: schemtoschematic,
    findUnknownBlocks: findUnknownBlocks,
  };
}
